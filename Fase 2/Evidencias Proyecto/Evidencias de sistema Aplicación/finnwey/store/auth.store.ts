...
/**
 * Helper para detectar si el error es por base de datos pausada o problemas de red
 */
function isDatabasePausedError(error: any): boolean {
  ...
  // Detectar errores comunes de base de datos pausada o problemas de red
  return (
    errorMessage.includes('network request failed') ||
    errorMessage.includes('not accepting connections') ||
    errorMessage.includes('57p03') ||
    errorMessage.includes('database system is not accepting') ||
    ...
    error.status === 503 ||
    error.status === 500 ||
    error.status === 0 // Error de red sin status
  )
}
...
function getFriendlyErrorMessage(error: any): string {
  if (isDatabasePausedError(error)) {
    return 'Error de conexión con el servidor. Por favor, verifica tu conexión a internet y vuelve a intentar. Si el problema persiste, la base de datos puede estar reactivándose.'
  }

  // Mensajes específicos de Supabase Auth
  if (error.message?.includes('Invalid login credentials')) {
    return 'Las credenciales no son válidas. Por favor, verifica tu correo y contraseña.'
  }
    ...
    (set, get) => ({
      ...
      login: async (credentials: LoginCredentials) => {
        ...
        try {
          ...
          // Si es un error de red, intentar limpiar la sesión localmente
          if (isDatabasePausedError(error)) {
            console.log('🔄 Detectado error de red, limpiando sesión local...')
            // No esperar a que termine, solo iniciar la limpieza
            clearAuthSession().catch((clearError) => {
              console.warn('⚠️ Error limpiando sesión (no crítico):', clearError)
            })
          }
      ...
      updateProfile: async (profileData: Partial<UserProfile>) => {
        ...
        try {
          // Filtrar valores null/undefined para evitar sobrescribir con null
          const cleanProfileData = Object.fromEntries(Object.entries(profileData).filter(([_, value]) => value !== null && value !== undefined))
          ...
          // Usar UPDATE en lugar de UPSERT para evitar problemas con campos requeridos
