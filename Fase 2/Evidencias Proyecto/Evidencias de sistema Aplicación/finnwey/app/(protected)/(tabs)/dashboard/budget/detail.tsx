import { View, Text, TouchableOpacity, ScrollView, Modal, TextInput, Alert, Pressable, ActivityIndicator } from 'react-native'
...
export default function BudgetDetailScreen() {
  ...
  const [floatingButtonState, setFloatingButtonState] = useState<{ show: boolean; isGenerating: boolean; onPress: () => void } | null>(null)
  ...
  // Calcular el gasto usando el método del hook, pero filtrando por el período del presupuesto
  const calculateSpentAmount = () => {
    if (!categoryId || !supabaseBudget) return 0
    
    const budgetStartDate = new Date(supabaseBudget.start_date)
    const budgetEndDate = new Date(supabaseBudget.end_date)
    
    return allTransactions
      .filter((t) => {
        // Filtrar por categoría y tipo de gasto
        if (t.category_id !== categoryId || t.categories?.type !== 'expense') {
          return false
        }
        
        // Filtrar por rango de fechas del presupuesto
  ...
  // Agregar spent_amount al presupuesto para que el componente BudgetDetailTabs lo use
  const budgetWithSpending = supabaseBudget
    ? {
        ...supabaseBudget,
        spent_amount: spentAmount,
        remaining_amount: supabaseBudget.amount - spentAmount,
      ...
      <SafeAreaView edges={['top']} className="flex-1">
        ...
        <View>
          <View className="flex-row justify-between items-center px-4 py-2">
            ...
            <Text className="text-white text-2xl font-medium">Detalle del Presupuesto</Text>
            <View>
              <TouchableOpacity onPress={() => setShowOptions(true)} className="w-12 h-12 bg-white/10 rounded-full items-center justify-center">
        ...
        {/* Contenido principal con tabs - sin ScrollView */}
        <View className="flex-1 bg-white dark:bg-gray-800 mt-2 rounded-t-3xl">
          <BudgetDetailTabs
            budget={budgetWithSpending as any}
            ...
            onFloatingButtonStateChange={setFloatingButtonState}
          />
        </View>

        {/* Botón flotante fijo - fuera del ScrollView */}
        {floatingButtonState?.show && (
          <Pressable
            onPress={floatingButtonState.onPress}
            disabled={floatingButtonState.isGenerating}
            className={`w-16 h-16 rounded-full items-center justify-center shadow-lg ${
              floatingButtonState.isGenerating
                ? isDarkMode
                  ? 'bg-gray-700'
                  : 'bg-gray-300'
                : isDarkMode
